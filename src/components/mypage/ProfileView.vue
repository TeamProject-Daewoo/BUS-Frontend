...
const fetchProfile = async () => {
  try {
    ...
    uiStore.openModal({message: "세션이 만료되었거나 로그인이 필요합니다."});
...
const submitUpdate = async () => {
  ...
  try {
    ...
    await fetchProfile();
    isEditing.value = false;
    uiStore.openModal({title:'수정이 완료되었습니다.'});
    passwords.value = { currentPassword: '', newPassword: '' };
  } catch (error) {
    console.error("프로필 수정에 실패했습니다.", error);
    uiStore.openModal({message:"프로필 수정 중 오류가 발생했습니다."});
  }
};
...
const handleLogout = async () => {
  try {
    ...
    uiStore.openModal({title:'로그아웃되었습니다.'});
    router.push('/loginview'); // 로그인 페이지 경로 확인
  } catch (error) {
...
watch(
  ...
  (isInitialized) => {
    ...
    if (isInitialized && authStore.accessToken) {
      ...
      uiStore.openModal({title:'로그인이 필요합니다.'});
